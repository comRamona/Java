...
public class Voronoi {
    ...
    public static int findClosestPoint(Point2D point, Point2D[] sites) {
        ...
        int n=sites.length;
        for(int i=0;i<n;i++)
        	if(pointDist(point,sites[i])<min) {min=pointDist(point,sites[i]); closest=i;}
        return closest;
    ...
    public static int[][] buildVoronoiMap(Point2D[] sites, int width, int height) {
    	...
    	for(int i=0;i<width;i++)
    		for(int j=0;j<height;j++)
    			{Point2D p=new Point2D(i,j); indexmap[i][j]=findClosestPoint(p,sites); }
    	return indexmap;
    }
    ...
    public static void plotVoronoiMap(Point2D[] sites, Color[] colors, int[][] indexMap) {
    	...
    	for(int i=0;i<width;i++)
    		for(int j=0;j<height;j++)
    		{ StdDraw.setPenColor(colors[indexMap[i][j]]); StdDraw.point(i,j);}
